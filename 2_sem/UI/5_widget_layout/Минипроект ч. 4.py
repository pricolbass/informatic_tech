...
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QLineEdit,
                             QListWidget, QLCDNumber, QTabWidget, QMessageBox, QCheckBox)
...
class DecisionHelper(QWidget):
    def __init__(self):
        ...
        self.questions = {}  # Теперь будет включать также состояние галочки "Решение принято"
    ...
    def initUI(self):
        ...
        # Добавляем галочку "Решение принято"
    ...
    def loadData(self):
        try:
            ...
            for question in self.questions.keys():
                tab = QWidget()
                tab.setObjectName(question)  # Устанавливаем имя объекта вкладки
    ...
    def addQuestion(self):
        ...
        if question and question not in self.questions:
            ...
            self.tabWidget.addTab(QWidget(), question)
            self.questionInput.clear()
            self.saveData()

    def changeQuestion(self):
        self.currentQuestion = self.tabWidget.currentWidget().objectName()
    ...
    def toggleDecisionState(self, state):
        if self.currentQuestion:
            self.questions[self.currentQuestion]["decisionMade"] = state == 2  # 2 is the state for a checked box
            self.updateUIState()
            self.saveData()
    ...
    def updateUIState(self):
        ...
        self.addQuestionButton.setEnabled(not decisionMade)
